          ...
          {isOpen && (
            <div className="lg:hidden py-4 border-t border-slate-200 max-h-[80vh] overflow-y-auto">
              ...
              <div className="px-4 py-4 space-y-2 border-t border-slate-200 mt-2">
                <Button variant="outline" className="w-full" onClick={() => { setIsLoginOpen(true); setIsOpen(false); }}>Login</Button>
                <Button className="w-full bg-gradient-to-r from-cyan-500 to-teal-600 text-white" onClick={() => { setIsBookingOpen(true); setIsOpen(false); }}>Book a Demo</Button>
              </div>
            </div>
          )}
        </div>
